//! Given a heap object implementing SyscallImpls, this module provides a
//! series of stub functions that provide the exact same implementation of
//! syscalls in https://docs.rs/ckb-std/latest/ckb_std/syscalls/index.html.
//!
//! This way, you can easily swap all the syscall invocations in a Rust based
//! CKB script to SyscallImpls based ones for certain purposes, such as
//! fuzzing.

use crate::SyscallImpls;
...
pub fn exec(index: usize, source: Source, place: usize, bounds: usize, argv: &[&CStr]) -> u64 {
    let result = get().exec(index, source.into(), place, bounds, argv);
    match result {
        Ok(_) => 0,
        Err(e) => e.into(),
    }
...
pub fn load_block_extension(
    ...
    get()
        .load_block_extension(buf, offset, index, source.into())
        .into()
}
...
pub fn load_cell(
    ...
    get().load_cell(buf, offset, index, source.into()).into()
}
...
pub fn load_cell_by_field(
    ...
    get()
        .load_cell_by_field(buf, offset, index, source.into(), field.into())
        .into()
}
...
pub fn load_cell_data(
    ...
    get()
        .load_cell_data(buf, offset, index, source.into())
        .into()
}
...
pub fn load_cell_data_raw(
    ...
    get()
        .load_cell_data_raw(buf_ptr, len, offset, index, source.into())
        .into()
}
...
pub fn load_header(
    ...
    get().load_header(buf, offset, index, source.into()).into()
}
...
pub fn load_header_by_field(
    ...
    get()
        .load_header_by_field(buf, offset, index, source.into(), field.into())
        .into()
}
...
pub fn load_input(
    ...
    get().load_input(buf, offset, index, source.into()).into()
}
...
pub fn load_input_by_field(
    ...
    get()
        .load_input_by_field(buf, offset, index, source.into(), field.into())
        .into()
}
...
pub fn load_witness(
    ...
    get().load_witness(buf, offset, index, source.into()).into()
}
...
pub fn spawn(
    ...
    let process_id = get().spawn(index, source.into(), place, bounds, &argv, &fds)?;
...
pub fn vm_version() -> Result<u64, SysError> {
    ...
    // Personally I think this logic is absurb but we will just keep
