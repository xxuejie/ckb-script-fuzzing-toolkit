...
impl SyscallImpls for StdSyscallWrapper {
    fn debug(&self, s: String) {
        std_syscalls::debug(s)
    }

    fn exit(&self, code: i8) -> ! {
        std_syscalls::exit(code)
    }

    fn load_cell(&self, buf: &mut [u8], offset: usize, index: usize, source: Source) -> IoResult {
        build_io_result(
            std_syscalls::load_cell(buf, offset, index, source.into()),
            buf.len(),
        )
    }
    ...
    fn load_cell_by_field(
        ...
        build_io_result(
            std_syscalls::load_cell_by_field(buf, offset, index, source.into(), field.into()),
            buf.len(),
        )
    }
    ...
    fn load_cell_code(
        ...
        ce(std_syscalls::load_cell_code(
            buf_ptr,
            len,
            content_offset,
            content_size,
    ...
    fn load_cell_data_raw(
        ...
        build_io_result(
            std_syscalls::load_cell_data_raw(buf_ptr, len, offset, index, source.into()),
            len,
        )
    }
    ...
    fn load_cell_data(
        ...
        build_io_result(
            std_syscalls::load_cell_data(buf, offset, index, source.into()),
            buf.len(),
        )
    }
    ...
    fn load_header(&self, buf: &mut [u8], offset: usize, index: usize, source: Source) -> IoResult {
        build_io_result(
            std_syscalls::load_header(buf, offset, index, source.into()),
            buf.len(),
        )
    }
    ...
    fn load_header_by_field(
        ...
        build_io_result(
            std_syscalls::load_header_by_field(buf, offset, index, source.into(), field.into()),
            buf.len(),
        )
    }
    ...
    fn load_input(&self, buf: &mut [u8], offset: usize, index: usize, source: Source) -> IoResult {
        build_io_result(
            std_syscalls::load_input(buf, offset, index, source.into()),
            buf.len(),
        )
    }
    ...
    fn load_input_by_field(
        ...
        build_io_result(
            std_syscalls::load_input_by_field(buf, offset, index, source.into(), field.into()),
            buf.len(),
        )
    }

    fn load_script(&self, buf: &mut [u8], offset: usize) -> IoResult {
        build_io_result(std_syscalls::load_script(buf, offset), buf.len())
    }

    fn load_script_hash(&self, buf: &mut [u8], offset: usize) -> IoResult {
        build_io_result(std_syscalls::load_script_hash(buf, offset), buf.len())
    }

    fn load_transaction(&self, buf: &mut [u8], offset: usize) -> IoResult {
        build_io_result(std_syscalls::load_transaction(buf, offset), buf.len())
    }

    fn load_tx_hash(&self, buf: &mut [u8], offset: usize) -> IoResult {
        build_io_result(std_syscalls::load_tx_hash(buf, offset), buf.len())
    }
    ...
    fn load_witness(
        ...
        build_io_result(
            std_syscalls::load_witness(buf, offset, index, source.into()),
            buf.len(),
        )
    }

    fn vm_version(&self) -> u64 {
        match std_syscalls::vm_version() {
            Ok(version) => version,
            Err(SysError::Unknown(future_version)) => future_version,
            Err(e) => panic!("Unexpected error: {:?}", e),
        }
    }

    fn current_cycles(&self) -> u64 {
        std_syscalls::current_cycles()
    }
    ...
    fn exec(
        ...
        let code = std_syscalls::exec(index, source.into(), place, bounds, argv);
        if code == 0 {
            Ok(())
        } else {
    ...
    fn spawn(
        ...
        let mut fds_with_terminator = vec![0; inherited_fds.len() + 1];
        ...
        let mut spgs = std_syscalls::SpawnArgs {
    ...
    fn load_block_extension(
        ...
        build_io_result(
            std_syscalls::load_block_extension(buf, offset, index, source.into()),
            buf.len(),
        )
    }
}

fn build_io_result(result: Result<usize, SysError>, original_length: usize) -> IoResult {
    match result {
        Ok(l) => {
...
fn ce<V>(result: Result<V, SysError>) -> Result<V, Error> {
